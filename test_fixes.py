...
if r.status_code == 201:
    ...
    print(f"3. Auth: FAIL {r.status_code} {r.text[:200]}")
    token = None

if token:
    headers = {"Authorization": f"Bearer {token}"}

    # Knowledge Base list (Fix #1)
    r = client.get(f"{BASE}/knowledge-base/", headers=headers)
    print(f"4. KB List (Fix #1): {r.status_code} OK")

    # Analytics
    r = client.get(f"{BASE}/analytics/costs", headers=headers)
